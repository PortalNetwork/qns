...
contract Registrar {
    ...
    modifier registryOpen() {
        if (now < registryStarted  || now > registryStarted + 4 years || qns.owner(rootNode) != address(this)) throw;
        _;
    }
     ...
     */
    function Registrar() {
        qns = AbstractQNS(36304e77b835948e29fafb359b214ec5b26ff700);
        rootNode = 0x9766c1e5363ec40fce6644f929cfd79730facc3a458add1a07fcc3b0938a0edd;
        registryStarted = 1526542132;
    }
    ...
    function trySetSubnodeOwner(bytes32 _hash, address _newOwner) internal {
        if (qns.owner(rootNode) == address(this))
            qns.setSubnodeOwner(rootNode, _hash, _newOwner);        
    }
    ...
    function releaseDeed(bytes32 _hash) onlyOwner(_hash) {
        ...
        if (now < h.registrationDate + 1 years && qns.owner(rootNode) == address(this)) throw;
    ...
    function _tryEraseSingleNode(bytes32 label) internal {
        if (qns.owner(rootNode) == address(this)) {
            qns.setSubnodeOwner(rootNode, label, address(this));
            var node = sha3(rootNode, label);
            qns.setResolver(node, 0);
            qns.setOwner(node, 0);
        }
    }
    ...
    function _eraseNodeHierarchy(uint idx, bytes32[] labels, bytes32 node) internal {
        ...
        qns.setSubnodeOwner(node, labels[idx], address(this));
    ...
    function transferRegistrars(bytes32 _hash) onlyOwner(_hash) {
        var registrar = qns.owner(rootNode);
    ...
    function changeMinPrice(uint _minPrice) {
        if(msg.sender != qns.owner(0)) throw;
        minPrice = _minPrice;
    }
