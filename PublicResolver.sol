...
contract PublicResolver {
    ...
    modifier only_owner(bytes32 node) {
        if (qns.owner(node) != msg.sender) throw;
        _;
    }
    ...
    function PublicResolver() {
        qns = AbstractQNS(0xb96836a066ef81ea038280c733f833f69c23efde);
    }
    ...
    /**
     * Returns the content hash associated with an QNS node.
    ...
    /**
     * Sets the content hash associated with an QNS node.
     * May only be called by the owner of that node in the QNS registry.
     * Note that this resource type is not standardized, and will likely change
     * in future to a resource type based on multihash.
     * @param node The node to update.
    ...
    /**
     * Returns the multihash associated with an QNS node.
     * @param node The QNS node to query.
    ...
    /**
     * Sets the multihash associated with an QNS node.
     * May only be called by the owner of that node in the QNS registry.
     * @param node The node to update.
    ...
    /**
     * Returns the SECP256k1 public key associated with an QNS node.
     * Defined in EIP 619.
     * @param node The QNS node to query
    ...
    /**
     * Sets the SECP256k1 public key associated with an QNS node.
     * @param node The QNS node to query
